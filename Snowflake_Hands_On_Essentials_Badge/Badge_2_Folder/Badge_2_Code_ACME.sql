...
SELECT COUNT(*)
from WEATHERSOURCE.STANDARD_TILE.HISTORY_DAY
join MARKETING.MAILERS.DETROIT_ZIPS
USING(postal_code);

with history AS (
    select MIN(DATE_VALID_STD) AS min_date, MAX(DATE_VALID_STD) AS max_date
    from history_day
    join MARKETING.MAILERS.DETROIT_ZIPS
    using(postal_code)
),
...
select date_valid_std, AVG(avg_cloud_cover_tot_pct) as avg_avg_cloud_cover
from forecast_day
join MARKETING.MAILERS.DETROIT_ZIPS
using(postal_code)
GROUP BY date_valid_std
ORDER BY avg_avg_cloud_cover ASC;
