...
-- Seperating as columns and not rows
...
-- 🥋 Recreate file format 1 to use elsewhere
...
select INITCAP(REPLACE(REPLACE(relative_path, '_', ' '), '.png')) as product_name,
    ...
    price,
    file_url
from directory(@sweatsuits) as di
join sweatsuits as sw
on di.relative_path = sw.file_name;
...
-- Query the Trails Geojson stage
select *
from @trails_geojson
(file_format => ff_json);
...
select 
    ...
    $1:latitude::number(11,8) as lng, -- comes through as latitude but is longitude
    $1:longitude::number(11,8) as lat, -- comes through as longitude but is latitude
...
-- replace function using the GEOGRAPHY argument
...
-- Super simple stripped fown external table
